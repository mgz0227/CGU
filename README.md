...
招生申请会写入 `cgu_admissions`（或内存存储），管理员后台提供“同意录取”与删除尚未建档申请的操作；同意后系统以幂等事务自动建立学生档案、生成学校邮箱、写入校内欢迎信，并尝试向申请人的联系邮箱发送不含初始密码的入学通知。学生门户使用 `studentEmailDomain` / `CGU_STUDENT_EMAIL_DOMAIN` 为已有学号的学生生成机构邮箱地址（默认 `@cgu.edu.kg`），并提供不依赖外部服务的校内收件箱。校内副本始终先保存，外发结果会记录为成功、失败、未配置或未知；超时、取消和服务重启后的未知状态必须由管理员确认后才可重试。初始密码只在首次审批响应中显示，服务端只保存 bcrypt 哈希；学生可以在个人资料页验证当前密码后轮换自己的密码，其他会话会被撤销。
...
- `/api/admin/*`：管理员统计与 CRUD 接口；`/api/admin/admissions` 查看招生申请，使用 `POST /api/admin/admissions/{id}/approve` 完成唯一的“同意”动作。首次同意会在同一事务中创建学生档案、生成校内邮箱并只在响应中返回一次初始密码；重复请求只返回已批准状态，不会再次生成或暴露密码。`DELETE /api/admin/admissions/{id}` 可删除尚未建立学生档案的申请，并同步清理关联的管理员通知。
...
- `/api/admin/admissions/{id}`：`PATCH/PUT` 只允许编辑联系资料和内部备注；申请决定不能通过状态字段绕过唯一的“同意”动作。`DELETE` 仅适用于尚未建立学生档案的申请；已自动建档并关联学号的申请返回 `409 admission_already_approved`，不会级联删除学生、成绩、课表或邮箱历史。
...
未配置 MySQL 时课程、公告、招生申请、学生目录、校内邮件和教务修改保存在进程内存中，服务重启后会重新加载初始内容；启用 MySQL 后首次启动会创建 `cgu_*` 表（包括 `cgu_admissions` 和 `cgu_mailbox_messages`）并写入缺失的课程、公告和内容目录。已有旧版邮箱表、招生表、用户表和课程表会在启动时按列幂等迁移，保留原有数据；课程英文列只在首次新增列时回填，管理员之后显式清空的值不会在重启后复原。版本更新中的官方至冬公告和首页日期只在数据库仍保存上一版内置值时做条件迁移，管理员已经编辑过的内容不会被覆盖。学生目录支持停用/启用账号：停用会立即撤销会话并阻止登录，但保留成绩、课表、邮箱和招生审计记录。SMTP 邮箱记录带唯一幂等键和数据库条件发送租约，支持单实例或共享 MySQL 的滚动重启；未配置数据库时只适合单进程运行。招生申请只在管理员接口返回，后台不提供状态下拉，必须使用唯一的 `POST /api/admin/admissions/{id}/approve` 同意动作；首次同意原子创建学生账号、校内邮箱和申请关联信息，并自动尝试外发入学通知，初始密码只响应一次。删除申请仅限尚未建立学生档案的记录，并与关联管理员通知在同一数据库事务中清理；已建档申请不会被级联删除。公开提交按客户端限流。学生账号由管理员创建或招生同意动作自动建立，服务端只保存密码哈希；学生可读取自己的已发布成绩、课表和自己的校内邮件，管理员接口要求会话、管理员角色、同源请求头和 CSRF 防护。网站内容编辑器允许保存空的中英文值来恢复随版本发布的内置文案；公告支持中英文正文和草稿状态，招生申请支持只改备注不改决定。后台和学生门户会定时刷新数据，单个接口失败时保留已成功加载的模块并显示可操作的部分失败提示。正式部署应使用 MySQL、HTTPS、反向代理、密钥管理和可观测的 SMTP 投递告警；多实例部署还应使用共享会话存储。静态服务器即使误指向部署目录也会拒绝配置、密钥、源码和数据库文件；生产仍应把 `CGU_STATIC_DIR` 指向专用 `web/` 目录，并以受限权限的绝对路径提供 `CGU_CONFIG_FILE` / `CGU_ENV_FILE`。
...
```powershell
./tools/New-CGURelease.ps1 -Tag v1.5.6 -NotesFile .\release-notes-v1.5.6.md
```
